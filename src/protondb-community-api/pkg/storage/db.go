...
func ConnectDB() error {
	...
	if err := client.Database("protondb").RunCommand(context.TODO(), bson.D{{Key: "ping", Value: 1}}).Err(); err != nil {
		panic(err)
	}
	fmt.Println("Pinged your deployment. You successfully connected to MongoDB!")

	gamesCollection = client.Database("protondb").Collection("games")
	reportsCollection = client.Database("protondb").Collection("reports")
	processStatusCollection = client.Database("protondb").Collection("process_status")
